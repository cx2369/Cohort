...
void cx_create_alias_table(afl_state_t *afl)
{

    afl->cx_alias_table_queue_items = afl->cx_valid_cxqueue_items;

    struct queue_entry **cx_alias_table_queue_buf = (struct queue_entry **)afl_realloc(((void **)&afl->cx_alias_table_queue_buf), afl->cx_alias_table_queue_items * sizeof(struct queue_entry *));
    if (unlikely(!cx_alias_table_queue_buf))
    {
        PFATAL("alloc");
    }
    u32 tem_i = 0;
    for (u32 i = 0; i < afl->cx_valid_cxqueue_items; i++)
    {
        cx_alias_table_queue_buf[tem_i] = afl->cx_valid_cxqueue_buf[i];
        tem_i++;
    }

    u32 n = afl->cx_alias_table_queue_items, i = 0, nSmall = 0, nLarge = n - 1;
    double sum = 0;
    ...
    afl->cx_alias_table =
        (u32 *)afl_realloc((void **)&afl->cx_alias_table, n * sizeof(u32));
    afl->cx_alias_probability = (double *)afl_realloc(
        (void **)&afl->cx_alias_probability, n * sizeof(double));

    if (!P || !Small || !Large || !afl->cx_alias_table || !afl->cx_alias_probability)
    {
        FATAL("could not acquire memory for alias table");
    }

    memset((void *)afl->cx_alias_probability, 0, n * sizeof(double));
    memset((void *)afl->cx_alias_table, 0, n * sizeof(u32));
    memset((void *)Small, 0, n * sizeof(u32));
    memset((void *)Large, 0, n * sizeof(u32));
    ...
    for (i = 0; i < n; i++)
    {
        struct queue_entry *q = afl->cx_alias_table_queue_buf[i];
        avg_exec_us += q->exec_us;
        avg_bitmap_size += log(q->bitmap_size);
        avg_len += q->len;
    }
    ...
    for (i = 0; i < n; i++)
    {
        P[i] = (afl->cx_alias_table_queue_buf[i]->weight * n) / sum;
    }
    ...
    while (nSmall && nLarge != n - 1)
    {
        ...
        afl->cx_alias_probability[small] = P[small];
        afl->cx_alias_table[small] = large;
    ...
    while (nSmall)
    {
        afl->cx_alias_probability[Small[--nSmall]] = 1;
    }

    while (nLarge != n - 1)
    {
        afl->cx_alias_probability[Large[++nLarge]] = 1;
    }

    afl->cx_reinit_table = 0;
}
...
void mark_as_redundant(afl_state_t *afl, struct queue_entry *q, u8 state)
{
    ...
    if (likely(state == q->fs_redundant))
    {
        return;
    }
...
static u8 check_if_text(afl_state_t *afl, struct queue_entry *q)
{
    ...
    if (q->len < AFL_TXT_MIN_LEN || q->len < AFL_TXT_MAX_LEN)
        return 0;
    ...
    if (len >= MAX_FILE)
        len = MAX_FILE - 1;
    if ((fd = open((char *)q->fname, O_RDONLY)) < 0)
        return 0;
...
void add_to_queue(afl_state_t *afl, u8 *fname, u32 len, u8 passed_det)
{
    ...
    cx_update_aflid_hnodeids(afl->cx_trace_bits, q->id);
...
void cx_add_to_queue(afl_state_t *afl, u8 *fname, u32 len, u8 passed_det)
{
    ...
    afl->latest_cxqueue = q;

    struct queue_entry **cx_total_cxqueue_buf = (struct queue_entry **)afl_realloc(((void **)&afl->cx_total_cxqueue_buf), afl->cx_total_cxqueue_items * sizeof(struct queue_entry *));
    if (unlikely(!cx_total_cxqueue_buf))
    {
        PFATAL("alloc");
    }
    cx_total_cxqueue_buf[afl->cx_total_cxqueue_items - 1] = q;

    q->cxid = afl->cx_queue_id;

    cx_update_cxid_hnodeids(afl->cx_trace_bits, q->cxid);
...
void destroy_queue(afl_state_t *afl)
{
    ...
    for (i = 0; i < afl->queued_items; i++)
    {
        ...
        if (q->skipdet_e)
        {
            ...
            if (q->skipdet_e->done_inf_map)
                ck_free(q->skipdet_e->done_inf_map);
            if (q->skipdet_e->skip_eff_map)
                ck_free(q->skipdet_e->skip_eff_map);
    ...
    for (u32 i = 0; i < afl->cx_total_cxqueue_items; i++)
    {
        if (afl->cx_total_cxqueue_buf[i] != NULL)
        {
            struct queue_entry *q;
            q = afl->cx_total_cxqueue_buf[i];
            ck_free(q->fname);
            ck_free(q->trace_mini);
            if (q->testcase_buf != NULL)
            {
    ...
    for (i = 0; i < afl->fsrv.map_size; ++i)
    {
        ...
        if (afl->fsrv.trace_bits[i])
        {
            ...
            if (afl->top_rated[i])
            {
                ...
                if (likely(fuzz_p2 > top_rated_fuzz_p2))
                {
                    continue;
                }

                if (likely(fav_factor > top_rated_fav_factor))
                {
                    continue;
                }
...
void cx_update_bitmap_score(afl_state_t *afl, struct queue_entry *q)
{
    ...
    fav_factor = q->factor_us_len;

    for (i = 0; i < afl->fsrv.map_size; ++i)
    {
        if (afl->fsrv.trace_bits[i])
        {
            if (afl->cx_top_rated[i])
            {
                u64 top_rated_fav_factor;
                top_rated_fav_factor = afl->cx_top_rated[i]->factor_us_len;
                if (likely(fav_factor > top_rated_fav_factor))
                {
                    continue;
                }
                if (!--afl->cx_top_rated[i]->tc_ref)
                {
                    ck_free(afl->cx_top_rated[i]->trace_mini);
                    afl->cx_top_rated[i]->trace_mini = 0;
                }
            }
            afl->cx_top_rated[i] = q;
            ++q->tc_ref;
            if (!q->trace_mini)
            {
                u32 len = (afl->fsrv.map_size >> 3);
            ...
            afl->cx_score_changed = 1;
        }
    }

    u64 end_time = get_cur_time();
    afl->total_cx_update2_time += end_time - start_time;
}
...
void cull_queue(afl_state_t *afl)
{
    ...
    if (likely(!afl->score_changed || afl->non_instrumented_mode))
    {
        return;
    }
    ...
    for (i = 0; i < afl->queued_items; i++)
    {
        ...
        afl->queue_buf[i]->not_free_trace_mini_label = 0;
    }
    afl->smallest_favored = -1;

    if (afl->cx_cov_all_edge_cxqueue_items)
    {
        memcpy(temp_v, afl->cx_map_tmp_buf, len);
    }
    ...
    //     {
    //         afl->top_rated[i]->not_free_trace_mini_label = 1;
    //     }
    // }

    // for (i = 0; i < afl->queued_items; i++)
    // {
    //     if (afl->queue_buf[i]->not_free_trace_mini_label == 0)
    //     {
    //         if (afl->queue_buf[i]->trace_mini)
    //         {
    //             ck_free(afl->queue_buf[i]->trace_mini);
    //             afl->queue_buf[i]->trace_mini = 0;
    //         }
    //     }
    // }
...
//  after new original afl queue
void cx_update_1(afl_state_t *afl)
{
    if (likely(!afl->cx_update_1))
    {
        return;
    }
    afl->cx_update_1 = 0;
    cx_update_edge_time(afl->virgin_bits_for_edge_time, afl->virgin_bits);
}

int cxcompare1(const void *a, const void *b)
{
    const struct queue_entry *entry_a = *(const struct queue_entry **)a;
    const struct queue_entry *entry_b = *(const struct queue_entry **)b;

    // uint64_t factor_a = (entry_a != NULL) ? entry_a->factor_us_len_bit : UINT64_MAX;
    // uint64_t factor_b = (entry_b != NULL) ? entry_b->factor_us_len_bit : UINT64_MAX;
    uint64_t factor_a = (entry_a != NULL) ? entry_a->factor_us_len : UINT64_MAX;
    uint64_t factor_b = (entry_b != NULL) ? entry_b->factor_us_len : UINT64_MAX;

    return (factor_a > factor_b) - (factor_a < factor_b);
}

//  after new cx queue or new edge
void cx_update_2(afl_state_t *afl)
{

    if (likely(!afl->cx_update_2))
    {
        return;
    }
    u64 start_time = get_cur_time();
    if (start_time < afl->next_cx_update2_time)
    {
        return;
    }
    ...
    afl->cx_reinit_table = 1;

    //  set cx valid cxqueue buf
    if (1)
    {
        afl->cx_valid_cxqueue_items = 0;
        for (u32 i = 0; i < afl->cx_total_cxqueue_items; i++)
        {
            if (afl->cx_total_cxqueue_buf[i] != NULL)
            {
                afl->cx_valid_cxqueue_items++;
            }
        }
        struct queue_entry **cx_valid_cxqueue_buf = (struct queue_entry **)afl_realloc(((void **)&afl->cx_valid_cxqueue_buf), afl->cx_valid_cxqueue_items * sizeof(struct queue_entry *));
        if (unlikely(!cx_valid_cxqueue_buf))
        {
            PFATAL("alloc");
        }
        u32 tem_i = 0;
        for (u32 i = 0; i < afl->cx_total_cxqueue_items; i++)
        {
            if (afl->cx_total_cxqueue_buf[i] != NULL)
            {
                cx_valid_cxqueue_buf[tem_i] = afl->cx_total_cxqueue_buf[i];
                tem_i++;
            }
        }
    }

    //  favored clear and not free label clear
    for (u32 i = 0; i < afl->cx_valid_cxqueue_items; i++)
    {
        afl->cx_valid_cxqueue_buf[i]->favored_level = 0;
        afl->cx_valid_cxqueue_buf[i]->favored = 0;
        afl->cx_valid_cxqueue_buf[i]->not_free_label = 0;
    }
    afl->cx_queued_favored = 0;
    afl->cx_pending_favored = 0;

    //  cx_valid_cxqueue_buf sort
    if (afl->cx_valid_cxqueue_items > 1)
    {
        qsort(afl->cx_valid_cxqueue_buf, afl->cx_valid_cxqueue_items, sizeof(struct queue_entry *), cxcompare1);
    }
    ...
    {
        cx_bak_hnodeids();
        afl->cx_cov_all_hnode_cxqueue_items = 0;
        for (u32 i = 0; i < afl->cx_valid_cxqueue_items; i++)
        {
            u8 ret = 0;
            ret = cx_cxqueueid_cov_all_hnode(afl->cx_valid_cxqueue_buf[i]->cxid);
            if (ret)
            {
                afl->cx_valid_cxqueue_buf[i]->favored_level |= 16;
                afl->cx_cov_all_hnode_cxqueue_items++;
            }
        }
        struct queue_entry **cx_cov_all_hnode_cxqueue_buf = (struct queue_entry **)afl_realloc(((void **)&afl->cx_cov_all_hnode_cxqueue_buf), afl->cx_cov_all_hnode_cxqueue_items * sizeof(struct queue_entry *));
        if (unlikely(!cx_cov_all_hnode_cxqueue_buf))
        {
            PFATAL("alloc");
        }
        u32 tem_i = 0;
        for (u32 i = 0; i < afl->cx_valid_cxqueue_items; i++)
        {
            if (afl->cx_valid_cxqueue_buf[i]->favored_level & 16)
            {
                cx_cov_all_hnode_cxqueue_buf[tem_i] = afl->cx_valid_cxqueue_buf[i];
                tem_i++;
            }
        }
    }

    //  cx_cov_all_edge_cxqueue_buf
    afl->ok_to_delete_useless = 0;
    if (afl->undeal_delete_useless_count > 5)
    {
        afl->cx_score_changed = 1;
        afl->undeal_delete_useless_count = 0;
    }
    if (1)
    {
        if (afl->cx_score_changed)
        {
            afl->ok_to_delete_useless = 1;
            afl->score_changed = 1;
            afl->cx_cov_all_edge_cxqueue_items = 0;
            u32 len = (afl->fsrv.map_size >> 3);
            u8 *temp_v = afl->cx_map_tmp_buf;
            afl->cx_score_changed = 0;
            memset(temp_v, 255, len);

            for (u32 i = 0; i < afl->fsrv.map_size; ++i)
            {
                if (afl->cx_top_rated[i] && (temp_v[i >> 3] & (1 << (i & 7))))
                {
                    u32 j = len;
                    while (j--)
                    {
                        if (afl->cx_top_rated[i]->trace_mini[j])
                        {
                            temp_v[j] &= ~afl->cx_top_rated[i]->trace_mini[j];
                        }
                    }
                    afl->cx_top_rated[i]->favored_level |= 1;
                }
            }
            for (u32 i = 0; i < afl->cx_valid_cxqueue_items; i++)
            {
                if (afl->cx_valid_cxqueue_buf[i]->favored_level & 1)
                {
                    afl->cx_cov_all_edge_cxqueue_items++;
                }
            }
            struct queue_entry **cx_cov_all_edge_cxqueue_buf = (struct queue_entry **)afl_realloc(((void **)&afl->cx_cov_all_edge_cxqueue_buf), afl->cx_cov_all_edge_cxqueue_items * sizeof(struct queue_entry *));
            if (unlikely(!cx_cov_all_edge_cxqueue_buf))
            {
                PFATAL("alloc");
            }
            u32 tem_i = 0;
            for (u32 i = 0; i < afl->cx_valid_cxqueue_items; i++)
            {
                if (afl->cx_valid_cxqueue_buf[i]->favored_level & 1)
                {
                    cx_cov_all_edge_cxqueue_buf[tem_i] = afl->cx_valid_cxqueue_buf[i];
                    tem_i++;
                }
            }
        }
        ...
        {
            afl->undeal_delete_useless_count++;
            for (u32 i = 0; i < afl->cx_cov_all_edge_cxqueue_items; i++)
            {
                afl->cx_cov_all_edge_cxqueue_buf[i]->favored_level |= 1;
            }
        }
        //  make sure cx_top_rated[i] do not destroy
        for (u32 i = 0; i < afl->fsrv.map_size; ++i)
        {
            if (afl->cx_top_rated[i])
            {
                afl->cx_top_rated[i]->not_free_label = 1;
            }
        }
    }
    if (1)
    {
        for (u32 i = 0; i < afl->cx_valid_cxqueue_items; i++)
        {
            afl->cx_valid_cxqueue_buf[i]->not_free_trace_mini_label = 0;
        }
        for (u32 i = 0; i < afl->fsrv.map_size; ++i)
        {
            if (afl->cx_top_rated[i])
            {
                afl->cx_top_rated[i]->not_free_trace_mini_label = 1;
            }
        }
        for (u32 i = 0; i < afl->cx_valid_cxqueue_items; i++)
        {
            if (afl->cx_valid_cxqueue_buf[i]->not_free_trace_mini_label == 0)
            {
                if (afl->cx_valid_cxqueue_buf[i]->trace_mini)
                {
                    ck_free(afl->cx_valid_cxqueue_buf[i]->trace_mini);
                    afl->cx_valid_cxqueue_buf[i]->trace_mini = 0;
                }
            }
        }
    }

    //  cx_cov_better_progress_cxqueue_buf
    if (1)
    {
        cx_set_cov_all_better_progress_cxid();
        afl->cx_cov_better_progress_cxqueue_items = 0;

        for (u32 i = 0; i < afl->cx_valid_cxqueue_items; i++)
        {
            u8 ret = 0;
            ret = cx_cxqueueid_cov_better_progress(afl->cx_valid_cxqueue_buf[i]->cxid);
            if (ret)
            {
                afl->cx_valid_cxqueue_buf[i]->favored_level |= 2;
                afl->cx_cov_better_progress_cxqueue_items++;
            }
        }
        struct queue_entry **cx_cov_better_progress_cxqueue_buf = (struct queue_entry **)afl_realloc(((void **)&afl->cx_cov_better_progress_cxqueue_buf), afl->cx_cov_better_progress_cxqueue_items * sizeof(struct queue_entry *));
        if (unlikely(!cx_cov_better_progress_cxqueue_buf))
        {
            PFATAL("alloc");
        }
        u32 tem_i = 0;
        for (u32 i = 0; i < afl->cx_valid_cxqueue_items; i++)
        {
            if ((afl->cx_valid_cxqueue_buf[i]->favored_level & 2))
            {
                cx_cov_better_progress_cxqueue_buf[tem_i] = afl->cx_valid_cxqueue_buf[i];
                tem_i++;
            }
        }
    }

    //  cx_cov_all_state_cxqueue_buf
    if (1)
    {
        cx_set_cov_all_state_cxid();
        afl->cx_cov_all_state_cxqueue_items = 0;
        for (u32 i = 0; i < afl->cx_valid_cxqueue_items; i++)
        {
            u8 ret = 0;
            ret = cx_cxqueueid_cov_all_state(afl->cx_valid_cxqueue_buf[i]->cxid);
            if (ret)
            {
                afl->cx_valid_cxqueue_buf[i]->favored_level |= 4;
                afl->cx_cov_all_state_cxqueue_items++;
            }
        }
        struct queue_entry **cx_cov_all_state_cxqueue_buf = (struct queue_entry **)afl_realloc(((void **)&afl->cx_cov_all_state_cxqueue_buf), afl->cx_cov_all_state_cxqueue_items * sizeof(struct queue_entry *));
        if (unlikely(!cx_cov_all_state_cxqueue_buf))
        {
            PFATAL("alloc");
        }
        u32 tem_i = 0;
        for (u32 i = 0; i < afl->cx_valid_cxqueue_items; i++)
        {
            if ((afl->cx_valid_cxqueue_buf[i]->favored_level & 4))
            {
                cx_cov_all_state_cxqueue_buf[tem_i] = afl->cx_valid_cxqueue_buf[i];
                tem_i++;
            }
        }
    }

    //  cx_cov_all_hash_cxqueue_buf
    if (1)
    {
        cx_set_cov_all_hash_cxid();
        afl->cx_cov_all_hash_cxqueue_items = 0;
        for (u32 i = 0; i < afl->cx_valid_cxqueue_items; i++)
        {
            u8 ret = 0;
            ret = cx_cxqueueid_cov_all_hash(afl->cx_valid_cxqueue_buf[i]->cxid);
            if (ret)
            {
                afl->cx_valid_cxqueue_buf[i]->favored_level |= 8;
                afl->cx_cov_all_hash_cxqueue_items++;
            }
        }
        struct queue_entry **cx_cov_all_hash_cxqueue_buf = (struct queue_entry **)afl_realloc(((void **)&afl->cx_cov_all_hash_cxqueue_buf), afl->cx_cov_all_hash_cxqueue_items * sizeof(struct queue_entry *));
        if (unlikely(!cx_cov_all_hash_cxqueue_buf))
        {
            PFATAL("alloc");
        }
        u32 tem_i = 0;
        for (u32 i = 0; i < afl->cx_valid_cxqueue_items; i++)
        {
            if ((afl->cx_valid_cxqueue_buf[i]->favored_level & 8))
            {
                cx_cov_all_hash_cxqueue_buf[tem_i] = afl->cx_valid_cxqueue_buf[i];
                tem_i++;
            }
        }
    }

    // choose a set of queue cov all edge execute count.
    if (1)
    {
        cx_set_cov_all_edge_count_cxid(afl->virgin_bits);
        afl->cx_cov_all_edge_count_cxqueue_items = 0;
        for (u32 i = 0; i < afl->cx_valid_cxqueue_items; i++)
        {
            u8 ret = 0;
            ret = cx_cxqueueid_cov_all_edge_count(afl->cx_valid_cxqueue_buf[i]->cxid);
            if (ret)
            {
                afl->cx_valid_cxqueue_buf[i]->favored_level |= 32;
                afl->cx_cov_all_edge_count_cxqueue_items++;
            }
        }
        struct queue_entry **cx_cov_all_edge_count_cxqueue_buf = (struct queue_entry **)afl_realloc(((void **)&afl->cx_cov_all_edge_count_cxqueue_buf), afl->cx_cov_all_edge_count_cxqueue_items * sizeof(struct queue_entry *));
        if (unlikely(!cx_cov_all_edge_count_cxqueue_buf))
        {
            PFATAL("alloc");
        }
        u32 tem_i = 0;
        for (u32 i = 0; i < afl->cx_valid_cxqueue_items; i++)
        {
            if ((afl->cx_valid_cxqueue_buf[i]->favored_level & 32))
            {
                cx_cov_all_edge_count_cxqueue_buf[tem_i] = afl->cx_valid_cxqueue_buf[i];
                tem_i++;
            }
        }
    }

    //  remove useless cxqueue
    uint32_t need_update_cx_valid_cxqueue = 0;
    if (1)
    {
        if (afl->ok_to_delete_useless)
        {
            for (u32 i = 0; i < afl->cx_total_cxqueue_items; i++)
            {
                if (afl->cx_total_cxqueue_buf[i] != NULL)
                {
                    if (afl->cx_total_cxqueue_buf[i]->favored_level == 0 && afl->cx_total_cxqueue_buf[i]->not_free_label == 0)
                    {
                        need_update_cx_valid_cxqueue = 1;
                        // cx_delete_cxqueueid_hnoderuninfos_with_id(afl->cx_total_cxqueue_buf[i]->cxid);
                        cx_delete_cxid_hnodeids_with_id(afl->cx_total_cxqueue_buf[i]->cxid);
                        cx_delete_cxid_edgeid_fuzzcount_with_id(afl->cx_total_cxqueue_buf[i]->cxid);
                        struct queue_entry *q;
                        q = afl->cx_total_cxqueue_buf[i];
                        if (q->testcase_buf != NULL)
                        {
                            ...
                            for (u32 j = 0; j < afl->q_testcase_max_cache_count; j++)
                            {
                                if (afl->q_testcase_cache[j] == q)
                                {
                                    afl->q_testcase_cache_size -= q->len;
                                    afl->q_testcase_cache[j] = NULL;
                                    if (j < afl->q_testcase_smallest_free)
                                    {
                                        afl->q_testcase_smallest_free = j;
                                    }
                                }
                            }
                        }
                        ...
                        ck_free(q);
                        afl->latest_cxqueue = NULL;
                        afl->cx_total_cxqueue_buf[i] = NULL;
                    }
                }
            }
        }
    ...
    //  update cx_valid_cxqueue
    if (need_update_cx_valid_cxqueue)
    {
        afl->cx_valid_cxqueue_items = 0;
        for (u32 i = 0; i < afl->cx_total_cxqueue_items; i++)
        {
    ...
    //  set cx_pending_not_fuzzed
    afl->cx_pending_not_fuzzed = 0;
    for (u32 i = 0; i < afl->cx_valid_cxqueue_items; i++)
    {
        if (!afl->cx_valid_cxqueue_buf[i]->was_fuzzed)
        {
            afl->cx_pending_not_fuzzed++;
        }
    }

    // set cx_cov_all_edge_cxqueue_not_fuzzed
    afl->cx_cov_all_edge_cxqueue_not_fuzzed = 0;
    for (u32 i = 0; i < afl->cx_cov_all_edge_cxqueue_items; i++)
    {
        if (!afl->cx_cov_all_edge_cxqueue_buf[i]->was_fuzzed)
        {
            afl->cx_cov_all_edge_cxqueue_not_fuzzed++;
        }
    }
    ...
    if (1)
    {
        ...
        afl->cx_mix_queue_items = 0;
        afl->cx_mix_queue_items += afl->cx_valid_cxqueue_items;
        afl->cx_mix_queue_items += afl->queued_items;
        struct queue_entry **cx_mix_queue_buf = (struct queue_entry **)afl_realloc(((void **)&afl->cx_mix_queue_buf), afl->cx_mix_queue_items * sizeof(struct queue_entry *));
        if (unlikely(!cx_mix_queue_buf))
        {
            PFATAL("alloc");
        }
        u32 tem_i = 0;
        for (u32 i = 0; i < afl->cx_valid_cxqueue_items; i++)
        {
            cx_mix_queue_buf[tem_i] = afl->cx_valid_cxqueue_buf[i];
            if (cx_mix_queue_buf[tem_i]->len > 4)
            {
                afl->ready_for_splicing_count++;
            }
            tem_i++;
        ...
        {
            cx_mix_queue_buf[tem_i] = afl->queue_buf[i];
            if (cx_mix_queue_buf[tem_i]->len > 4)
            {
                afl->ready_for_splicing_count++;
            }
    ...
    //  set favored
    for (u32 i = 0; i < afl->cx_cov_all_hnode_cxqueue_items; i++)
    {
        afl->cx_cov_all_hnode_cxqueue_buf[i]->favored = 1;
        afl->cx_queued_favored++;
        if (!afl->cx_cov_all_hnode_cxqueue_buf[i]->was_fuzzed)
        {
            afl->cx_pending_favored++;
        }
    }
    // for (u32 i = 0; i < afl->cx_cov_all_edge_cxqueue_items; i++)
    // {
    //     afl->cx_cov_all_edge_cxqueue_buf[i]->favored = 1;
    //     afl->cx_queued_favored++;
    //     if (!afl->cx_cov_all_edge_cxqueue_buf[i]->was_fuzzed)
    //     {
    //         afl->cx_pending_favored++;
    //     }
    // }
    // if (afl->cx_pending_favored == 0)
    // {
    //     for (u32 i = 0; i < afl->cx_cov_better_progress_cxqueue_items; i++)
    //     {
    //         if (!afl->cx_cov_better_progress_cxqueue_buf[i]->favored)
    //         {
    //             if (!afl->cx_cov_better_progress_cxqueue_buf[i]->was_fuzzed)
    //             {
    //                 afl->cx_cov_better_progress_cxqueue_buf[i]->favored = 1;
    //                 afl->cx_queued_favored++;
    //                 afl->cx_pending_favored++;
    //             }
    //         }
    ...
    for (u32 i = 0; i < afl->cx_valid_cxqueue_items; i++)
    {
        afl->cx_valid_cxqueue_buf[i]->perf_score = cx_calculate_score(afl, afl->cx_valid_cxqueue_buf[i]);
    }

    u64 end_time = get_cur_time();
    afl->total_cx_update2_time += (end_time - start_time);
    u32 tem_k = 10 + (afl->fuzz_afl_nums + afl->fuzz_cx_nums) / 30;
    if (tem_k > 50)
    {
        tem_k = 50;
    }
    afl->next_cx_update2_time = end_time + tem_k * (end_time - start_time);
}
...
u32 calculate_score(afl_state_t *afl, struct queue_entry *q)
{
    ...
    if (unlikely(!cal_cycles))
    {
        cal_cycles = 1;
    }
    if (unlikely(!bitmap_entries))
    {
        bitmap_entries = 1;
    }
...
u32 cx_calculate_score(afl_state_t *afl, struct queue_entry *q)
{
...
double cx_calculate_weight(afl_state_t *afl, struct queue_entry *q)
{
    double weight = 1.0;
    ...
    // u32 tem_favored_count = 0;
    // tem_favored_count = __builtin_popcountll(q->favored_level);
    // for (u32 i = 0; i < tem_favored_count; i++)
    // {
    //     weight *= 1.1;
    // }

    if (q->favored_level & 1)
    {
        weight *= 1.1;
    }
    if (q->favored_level & 2)
    {
        weight *= 1.1;
    }
    if (q->favored_level & 4)
    {
        weight *= 1.1;
    }
    if (q->favored_level & 8)
    {
        weight *= 1;
    }
