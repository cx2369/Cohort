...
nyx_plugin_handler_t *afl_load_libnyx_plugin(u8 *libnyx_binary)
{
    ...
    if (plugin->nyx_config_load == NULL)
    {
        goto fail;
    }
    ...
    if (plugin->nyx_config_set_workdir_path == NULL)
    {
        goto fail;
    }
    ...
    if (plugin->nyx_config_set_input_buffer_size == NULL)
    {
        goto fail;
    }
    ...
    if (plugin->nyx_config_set_hprintf_fd == NULL)
    {
        goto fail;
    }
    ...
    if (plugin->nyx_config_set_process_role == NULL)
    {
        goto fail;
    }
    ...
    if (plugin->nyx_config_set_reuse_snapshot_path == NULL)
    {
        goto fail;
    }
    ...
    if (plugin->nyx_get_aux_string == NULL)
    {
        goto fail;
    }

    plugin->nyx_remove_work_dir = dlsym(handle, "nyx_remove_work_dir");
    if (plugin->nyx_remove_work_dir == NULL)
    {
        goto fail;
    }
    ...
    if (plugin->nyx_config_set_aux_buffer_size == NULL)
    {
        goto fail;
    }
                    ...
                    volatile u8 *stop_soon_p, u8 debug_child_output)
{
    ...
    if (unlikely(fsrv->nyx_mode))
    {
        ...
        if (!be_quiet)
        {
            ACTF("Spinning up the NYX backend...");
        }
        ...
        if (!be_quiet)
        {
            ACTF("Target map size: %u", fsrv->real_map_size);
        }
    ...
    if (fsrv->use_fauxsrv)
    {
        ...
        if (!be_quiet)
        {
            ACTF("Using AFL++ faux forkserver...");
        }
    ...
    if (rlen == 4)
    {
        ...
        if (status >= 0x41464c00 && status <= 0x41464cff)
        {
            ...
            if (status & FS_NEW_OPT_SHDMEM_FUZZ)
            {
                ...
                if (fsrv->support_shmem_fuzz)
                {
                    ...
                    if (!be_quiet)
                    {
                        ACTF("Using SHARED MEMORY FUZZING feature.");
                    }
                }
                else
                {
        ...
        else
        {
            ...
            if (!be_quiet)
            {
                OKF("All right - old fork server is up.");
            }
    ...
    if (waitpid(fsrv->fsrv_pid, &status, 0) <= 0)
    {
...
afl_fsrv_run_target(afl_forkserver_t *fsrv, u32 timeout, volatile u8 *stop_soon_p)
{
    ...
    if (fsrv->nyx_mode)
    {
        ...
        case InvalidWriteToPayload:
            if (!!getenv("AFL_NYX_HANDLE_INVALID_WRITE"))
            {
                return FSRV_RUN_CRASH;
            }
...
fsrv_run_result_t __attribute__((hot))
cx_afl_fsrv_run_target(afl_forkserver_t *fsrv, u32 timeout, volatile u8 *stop_soon_p, uint8_t *cx_trace_bits, uint32_t cx_map_size)
{
    ...
    {
        memset(fsrv->trace_bits, 0, fsrv->map_size);
        memset(cx_trace_bits, 0, cx_map_size);
        MEM_BARRIER();
    }
