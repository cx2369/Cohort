...
void aflcc_state_init(aflcc_state_t *aflcc, u8 *argv0)
{
    ...
    if (getenv("AFL_DEBUG"))
    {
        ...
        aflcc->debug = 1;
        if (strcmp(getenv("AFL_DEBUG"), "0") == 0)
            unsetenv("AFL_DEBUG");
    }
...
void mode_final_checkout(aflcc_state_t *aflcc, int argc, char **argv)
{
    ...
    case GCC:
        if (!aflcc->have_gcc)
            FATAL("afl-gcc is not available on your platform!");
        break;
    case CLANG:
        if (!aflcc->have_clang)
             ...
             strstr(cur_argv, "list="))
    {
        ...
        else
        {
            ...
            if (aflcc->instrument_mode != INSTRUMENT_LLVMNATIVE)
            {
                ...
                if (!be_quiet)
                {
                    WARNF("Found '%s' - stripping!", cur_argv);
                }
                final_ = PARAM_DROP;
            }
            else
...
void add_lto_linker(aflcc_state_t *aflcc)
{
    ...
    if (!ld_path)
    {
        PFATAL("Could not allocate mem for ld_path");
    }
...
#define AFL_AS_ERR "(should be a symlink or copy of 'afl-as')"
    ...
    if (fd < 0)
    {
        PFATAL("Unable to open '%s' " AFL_AS_ERR, afl_as_dup);
    }
                           ...
                           char **argv)
{
    ...
    while (--argc)
    {
        ...
        if (PARAM_MISS != parse_misc_params(aflcc, cur, scan))
            continue;

        if (PARAM_MISS != parse_fsanitize(aflcc, cur, scan))
            continue;
        ...
        */
        if (*cur == '@')
        {
            ...
            if (scan)
            {
                ...
                if (rsp_count == 0)
                    FATAL("Too many response files provided!");
...
#define ARG_STORE()                                                    \
    ...
    {                                                                  \
        ...
        arg_len = 0;                                                   \
            ...
            {
                ARG_STORE();
            } // save the pending arg after EOF
...
#undef ARG_STORE
            ...
            if (argc_read > 1)
            {
                process_params(aflcc, scan, argc_read, argv_read);
            }
...
int main(int argc, char **argv, char **envp)
{
    ...
    else
    {
        ...
        int i;
        for (i = 0; aflcc->cc_params[i]; i++)
        {
            // fprintf(stderr, "cc_params[%d]: %s\n", i, aflcc->cc_params[i]);
        }
