...
void cx_shm_init(afl_state_t *afl, u32 cx_map_size)
{
    afl->cx_shm_id = shmget(IPC_PRIVATE, cx_map_size, IPC_CREAT | IPC_EXCL | DEFAULT_PERMISSION);
    if (afl->cx_shm_id < 0)
    {
        PFATAL("cx_shm_id shmget() failed");
    }
    u8 *cx_shm_str;
    cx_shm_str = alloc_printf("%d", afl->cx_shm_id);
    setenv(CX_SHM_ENV_VAR, cx_shm_str, 1);
    ck_free(cx_shm_str);
    afl->cx_trace_bits = shmat(afl->cx_shm_id, NULL, 0);
    if (afl->cx_trace_bits == (void *)-1 || !(afl->cx_trace_bits))
    {
        shmctl(afl->cx_shm_id, IPC_RMID, NULL);
        PFATAL("cx_shm_id shmat() failed");
    }
}

void cx_shm_deinit(afl_state_t *afl)
{
    unsetenv(CX_SHM_ENV_VAR);
    shmctl(afl->cx_shm_id, IPC_RMID, NULL);
    afl->cx_trace_bits = NULL;
}

u8 cx_choose_afl_or_cx(afl_state_t *afl)
{

    u8 ret = 1;

    if (afl->cx_valid_cxqueue_items == 0)
    {
        return 1;
    }
    ...
    if (1)
    {
        u32 tem1 = afl->pending_favored;
        u32 tem2 = afl->cx_pending_favored;
        u32 tem3 = 0;
        tem3 = tem1 + tem2;
        if (tem3 && (rand_below(afl, 100) < 90))
        {
            u32 tem_rand_num1 = rand_below(afl, tem3);
            if (tem_rand_num1 < tem1)
            {
                return 1;
            }
            else
    ...
    if (1)
    {
        u32 tem1 = afl->queued_favored;
        u32 tem2 = afl->cx_queued_favored;
        u32 tem3 = 0;
        tem3 = tem1 + tem2;
        if (tem3 && (rand_below(afl, 100) < 75))
        {
            u32 tem_rand_num1 = rand_below(afl, tem3);
            if (tem_rand_num1 < tem1)
            {
    ...
    if (1)
    {
        u32 tem1 = afl->queued_items;
        u32 tem2 = afl->cx_valid_cxqueue_items;
        u32 tem3 = 0;
        tem3 = tem1 + tem2;
        if (tem3)
        {
...
struct queue_entry *cx_choose_from_cx(afl_state_t *afl)
{
    struct queue_entry *ret = NULL;

    u64 tem_time_s = (get_cur_time() - afl->last_find_time) / 1000;
    u64 tem_time_h1 = 200;
    u64 tem_time_h2 = 100;

    uint8_t tem_label = 0;
    if (tem_time_s > tem_time_h1)
    {
        if (rand_below(afl, 100) < 5)
        {
            tem_label = 1;
        }
    }

    //  smaller fuzzed hnode
    uint32_t smaller_fuzzed_hnodeid = 0;
    if (tem_label)
    {
        smaller_fuzzed_hnodeid = cx_get_smaller_fuzzed_hnodeid();
        if (smaller_fuzzed_hnodeid == 0)
        {
            tem_label = 0;
        }
    }
    ...
    //  alias table
    if (afl->cx_reinit_table == 1)
    {
        u64 tem_start_time = get_cur_time();
        cx_create_alias_table(afl);
        u64 tem_end_time = get_cur_time();
        afl->total_alias_time += (tem_end_time - tem_start_time);
    }
    u32 ig1 = 0, ig2 = 0, ig3 = 0;
    for (u32 i = 0; i < 99999; i++)
    {
        u32 s = rand_below(afl, afl->cx_alias_table_queue_items);
        double p = rand_next_percent(afl);
        u32 idx = p < afl->cx_alias_probability[s] ? s : afl->cx_alias_table[s];
        ret = afl->cx_alias_table_queue_buf[idx];
        if (afl->cx_pending_favored && !ig1)
        {
            if ((ret->fuzz_level || !ret->favored))
            {
                if (likely(rand_below(afl, 1000) < 999))
                {
                    continue;
                }
                else
        ...
        {
            if (afl->cx_cov_all_edge_cxqueue_not_fuzzed && !ig2)
            {
                if ((ret->fuzz_level || !(ret->favored_level & 1)))
                {
                    if (likely(rand_below(afl, 1000) < 999))
                    {
                        continue;
            ...
            {
                u32 tem_p = 0;
                if (ret->favored) //  all hnode
                {
                    tem_p = 0;
                }
                else if (ret->favored_level & 1) //  all edge
                {
                    tem_p = 50;
                }
                else if (ret->favored_level & 2) //  better progress
                {
                    tem_p = 65;
                    if (afl->queue_cycle > 1 && ret->fuzz_level == 0)
                    {
                        tem_p = 50;
                    }
                }
                else if (ret->favored_level & 4) // all state
                {
                    tem_p = 75;
                    if (afl->queue_cycle > 1 && ret->fuzz_level == 0)
                    {
                ...
                else if (ret->favored_level & 32) // all edge count
                {
                    tem_p = 95;
                    if (afl->queue_cycle > 1 && ret->fuzz_level == 0)
                    {
                ...
                else if (ret->favored_level & 8) // all hash
                {
                    tem_p = 95;
                    if (tem_time_s > tem_time_h2)
                    {
                        tem_p = 85;
                    }
                    if (afl->queue_cycle > 1 && ret->fuzz_level == 0)
                ...
                if (likely(rand_below(afl, 100) < tem_p))
                {
                    continue;
                }
            }
        ...
        if (tem_label && !ig3)
        {
            if (!cx_queue_reach_this_hnodeid(ret->cxid, smaller_fuzzed_hnodeid))
            {
                if (likely(rand_below(afl, 100) < 99))
                {
                    continue;
                }
                else
...
static void at_exit()
{
    ...
    if ((ptr = getenv("AFL_KILL_SIGNAL")))
    {
        kill_signal = atoi(ptr);
    }
...
int main(int argc, char **argv_orig, char **envp)
{
                         ...
                         "T:UV:WXx:YzZ")) > 0)
    {
        ...
        case 's':
        {
            ...
            if (optarg == NULL)
            {
                FATAL("No valid seed provided. Got NULL.");
            }
        ...
        case 'i': /* input dir */
            ...
            if (optarg == NULL)
            {
                FATAL("Invalid -i option (got NULL).");
            }
            afl->in_dir = optarg;

            if (!strcmp(afl->in_dir, "-"))
            {
        ...
        case 'O': /* FRIDA mode */
            ...
            if (get_afl_env("AFL_USE_FASAN"))
            {
                afl->fsrv.frida_asan = 1;
            }
        ...
        case 'Q': /* QEMU mode */
            ...
            if (!mem_limit_given)
            {
                afl->fsrv.mem_limit = MEM_LIMIT_QEMU;
            }
        ...
        case 'U': /* Unicorn mode */
            ...
            if (!mem_limit_given)
            {
                afl->fsrv.mem_limit = MEM_LIMIT_UNICORN;
            }
    ...
    if (afl->fsrv.mem_limit && afl->shm.cmplog_mode)
        afl->fsrv.mem_limit += 260;
    ...
    if (afl->non_instrumented_mode)
    {
        ...
        if (afl->fsrv.frida_mode)
        {
            FATAL("-O and -n are mutually exclusive");
        }
        if (afl->fsrv.qemu_mode)
        {
        ...
        if (afl->fsrv.cs_mode)
        {
            FATAL("-A and -n are mutually exclusive");
        }
        if (afl->unicorn_mode)
        {
    ...
    if (get_afl_env("AFL_NO_FORKSRV"))
    {
        afl->no_forkserver = 1;
    }
    if (get_afl_env("AFL_NO_CPU_RED"))
    {
        afl->no_cpu_meter_red = 1;
    }
    if (get_afl_env("AFL_NO_ARITH"))
    {
        afl->no_arith = 1;
    }
    if (get_afl_env("AFL_SHUFFLE_QUEUE"))
    {
        afl->shuffle_queue = 1;
    }
    if (get_afl_env("AFL_EXPAND_HAVOC_NOW"))
    {
        afl->expand_havoc = 1;
    }
    ...
    if (afl->afl_env.afl_hang_tmout)
    {
        ...
        if (hang_tmout < 1)
        {
            FATAL("Invalid value for AFL_HANG_TMOUT");
        }
    ...
    if (afl->afl_env.afl_max_det_extras)
    {
        ...
        if (max_det_extras < 1)
        {
            FATAL("Invalid value for AFL_MAX_DET_EXTRAS");
        }
        ...
        afl->fsrv.frida_mode || afl->fsrv.cs_mode || afl->unicorn_mode)
    {
        ...
        afl->cx_top_rated = ck_realloc(afl->cx_top_rated, map_size * sizeof(void *));
        ...
        afl->cx_map_tmp_buf = ck_realloc(afl->cx_map_tmp_buf, map_size);
        ...
        if (old_map_size < map_size)
        {
            ...
            memset(afl->cx_top_rated + old_map_size, 0, map_size - old_map_size);
            ...
            memset(afl->cx_map_tmp_buf + old_map_size, 0, map_size - old_map_size);
        }
    }

    afl->argv = use_argv;
    afl->fsrv.trace_bits = afl_shm_init(&afl->shm, afl->fsrv.map_size, afl->non_instrumented_mode);
        ...
        !afl->afl_env.afl_skip_bin_check)
    {
        ...
        //  get recompile path
        //  deal dictionary
        afl->put_recompile_path = (u8 *)malloc(1024 * sizeof(u8));
        if (afl->put_recompile_path == NULL)
        {
            FATAL();
        }
        printf("put path:%s\n", afl->fsrv.target_path);
        char put_simpleinfo1_path[1024];
        char put_dictfile_path[1024];
        if (strlen(afl->fsrv.target_path) > 512)
        {
            FATAL();
        }
        strcpy(put_simpleinfo1_path, afl->fsrv.target_path);
        char *last_slash = strrchr(put_simpleinfo1_path, '-');
        if (last_slash != NULL)
        {
            *last_slash = '\0';
            if (strlen(put_simpleinfo1_path) > 512)
            {
                FATAL();
            }
            strcpy(afl->put_recompile_path, put_simpleinfo1_path);
            strcpy(put_dictfile_path, put_simpleinfo1_path);
            strcat(put_simpleinfo1_path, "-tmp2-cx-simpleinfo1.txt");
            strcat(afl->put_recompile_path, "-recompile.sh");
            strcat(put_dictfile_path, "-tmp2-cx-dictfile");
        }
        else
        {
            FATAL("");
        }
        printf("put simple info file:%s\n", put_simpleinfo1_path);
        printf("put recompile script:%s\n", afl->put_recompile_path);
        printf("put dict file:%s\n", put_dictfile_path);

        if (1)
        {
            //  set dict
            FILE *file = fopen(put_dictfile_path, "rb");
            if (!file)
            {
                FATAL();
            }
            ...
            size_t length;
            while (fread(&length, sizeof(size_t), 1, file) == 1)
            {
                uint8_t *mem = (uint8_t *)malloc(length + 1);
                if (!mem)
                {
                    fclose(file);
                    FATAL();
                ...
                size_t read_count = fread(mem, sizeof(uint8_t), length, file);
        ...
        if (1)
        {
            FILE *file = fopen(put_simpleinfo1_path, "r");
            if (file == NULL)
            {
                FATAL();
            }
            ...
            {
                if (sscanf(line, "afl_final_loc: %d", &afl_final_loc) == 1)
                {
                    printf("afl_final_loc: %d\n", afl_final_loc);
                }
                if (sscanf(line, "total_used_bytes: %d", &cx_map_size) == 1)
                {
                    printf("cx_map_size: %d\n", cx_map_size);
                }
            }
            fclose(file);
        }
        afl_final_loc++;
        afl->afl_final_loc = afl_final_loc;
        afl->cx_map_size = cx_map_size;
        if (afl->cx_map_size < 32)
        {
            FATAL();
        }
        cx_shm_init(afl, afl->cx_map_size);

        u32 new_map_size = afl_fsrv_get_mapsize(&afl->fsrv, afl->argv, &afl->stop_soon, afl->afl_env.afl_debug_child);

        afl->fsrv.real_map_size = afl->afl_final_loc;
        afl->fsrv.map_size = afl->afl_final_loc;
        new_map_size = afl->afl_final_loc;
        ...
        if (map_size < new_map_size)
        {
            ...
            afl->virgin_bits_for_edge_time = ck_realloc(afl->virgin_bits_for_edge_time, new_map_size);
            ...
            afl->cx_top_rated =
                ck_realloc(afl->cx_top_rated, new_map_size * sizeof(void *));
            ...
            afl->cx_map_tmp_buf = ck_realloc(afl->cx_map_tmp_buf, new_map_size);
            ...
            if (old_map_size < new_map_size)
            {
                ...
                memset(afl->cx_top_rated + old_map_size, 0, new_map_size - old_map_size);
                ...
                memset(afl->cx_map_tmp_buf + old_map_size, 0, new_map_size - old_map_size);
            }

            afl_fsrv_kill(&afl->fsrv);
            afl_shm_deinit(&afl->shm);
            afl->fsrv.map_size = new_map_size;
            afl->fsrv.trace_bits = afl_shm_init(&afl->shm, new_map_size, afl->non_instrumented_mode);
    ...
    memset(afl->virgin_bits_for_edge_time, 255, map_size);

    //  cx init
    cx_set_afl_final_loc(afl->afl_final_loc);
    cx_init(afl->fsrv.target_path);
    cx_set_edge_count_best_cxid();
    cx_set_edge_id_fuzz_count();
    ...
    if (afl->q_testcase_max_cache_entries)
    {
        ...
        if (!afl->q_testcase_cache)
        {
            PFATAL("malloc failed for cache entries");
        }
    }
    ...
    if (1)
    {
        ...
        {
            q = afl->queue_buf[idx];
            if (unlikely(!q || q->disabled))
            {
                continue;
            }
            u8 *out_buf;
            out_buf = afl_realloc(AFL_BUF_PARAM(out), q->len);
            if (unlikely(!out_buf))
            {
                PFATAL("alloc");
            }
            out_buf = queue_testcase_get(afl, q);
            (void)write_to_testcase(afl, (void **)&out_buf, q->len, 1);
            ...
            cx_update_queueid_edgeid(q->afl_or_cx, q->id, afl->fsrv.trace_bits, afl->fsrv.map_size);
        }
    }

    // real start time, we reset, so this works correctly with -V
    update_after_new_edge(afl->virgin_bits);
    afl->start_time = get_cur_time();
    afl->choose_afl_or_cx = 1;
    afl->orig_queued_items = afl->queued_items;

    //  cx variables for while
    uint64_t recompile_interval_time = 6 * 60 * 1000;
    uint64_t recompile_interval_execs = 300 * 5 * 60;
    uint64_t next_recompile_time = get_cur_time() + recompile_interval_time;
    uint64_t cx_start_time = 1 * 30 * 1000;
    ...
    {
        // update_after_new_edge(afl->virgin_bits);
        // cx_delete_cur_HNodeInfoRuns();
    }

    //  cxwhile
    while (likely(!afl->stop_soon))
    {
        cx_delete_after_new_edge();
        cx_update_1(afl);
        cx_update_2(afl);
        cull_queue(afl);

        //  cx start
        if (get_cur_time() - afl->start_time > cx_start_time && afl->cx_start == 0)
        {
            afl->cx_start = 1;
            update_after_new_edge(afl->virgin_bits);
            // cx_delete_cur_HNodeInfoRuns();
            // cx_set_cur_edge_useless();
        }

        //  recompile

        if (get_cur_time() > next_recompile_time)
        {
            next_recompile_time += 3 * 60 * 1000;
            if (afl->fsrv.total_execs - afl->cx_last_recompile_total_execs > recompile_interval_execs || afl->recompile_count == 0)
            {
                afl->cx_last_recompile_total_execs = afl->fsrv.total_execs;
                cx_clear_fuzzed_cxid();
                for (u32 tem_i = 0; tem_i < afl->cx_valid_cxqueue_items; tem_i++)
                {
                    if (afl->cx_valid_cxqueue_buf[tem_i]->fuzz_level > 0 || afl->cx_valid_cxqueue_buf[tem_i]->cal_failed)
                    {
                        cx_update_fuzzed_cxid(afl->cx_valid_cxqueue_buf[tem_i]->cxid);
                    }
                }
                cx_update_has_hnode_cov_not_fuzzed();
                cx_update_afl_useless_id();
                cx_update_cx_useless_id();
                uint32_t should_recompile = 0;
                should_recompile = cx_should_recompile();
                if (should_recompile)
                {
                    //  after recompile, exec cksum will change
                    for (u32 i = 0; i < afl->queued_items; i++)
                    {
                        afl->queue_buf[i]->exec_cksum = 0;
                    }
                    for (u32 i = 0; i < afl->cx_total_cxqueue_items; i++)
                    {
                        if (afl->cx_total_cxqueue_buf[i] != NULL)
                        {
                            afl->cx_total_cxqueue_buf[i]->exec_cksum = 0;
                        }
                    }
                    afl_fsrv_kill(&afl->fsrv);
                    if (afl->shm.cmplog_mode)
                    ...
                    cx_shm_deinit(afl);
                    //  recompile start
                    cx_write_solved_id_to_file();
                    cx_write_useless_edge_id_to_file();
                    u64 recompile_start_time_ms = get_cur_time();
                    int result = system((char *)afl->put_recompile_path);
                    if (result == -1)
                    {
                        FATAL();
                    }
                    ...
                    u64 recompile_end_time_ms = get_cur_time();
                    afl->total_recompile_ms += (recompile_end_time_ms - recompile_start_time_ms);
                    afl->recompile_count++;
                    u64 avg_recompile_ms = afl->total_recompile_ms / afl->recompile_count;
                    u64 tem_time1 = avg_recompile_ms * (50 + 3 * afl->recompile_count);
                    next_recompile_time = get_cur_time() + tem_time1;
                    //  recompile end
                    afl->fsrv.real_map_size = get_map_size();
                    afl->fsrv.map_size = get_map_size();
                    if (afl->fsrv.map_size < afl->afl_final_loc)
                    {
                        FATAL();
                    }
                    check_binary(afl, afl->fsrv.target_path);
                    afl->fsrv.trace_bits = afl_shm_init(&afl->shm, afl->fsrv.map_size, afl->non_instrumented_mode);
                    if (afl->shm.cmplog_mode)
                    {
                        afl->cmplog_fsrv.trace_bits = afl->fsrv.trace_bits;
                    ...
                    if (afl->shm.cmplog_mode)
                    {
                        afl_fsrv_start(&afl->cmplog_fsrv, afl->argv, &afl->stop_soon, afl->afl_env.afl_debug_child);
                    }
                    afl->fsrv.real_map_size = afl->afl_final_loc;
                    afl->fsrv.map_size = afl->afl_final_loc;
                    printf("\033[H\033[J");
        ...
        if (unlikely((!afl->old_seed_selection &&
                      runs_in_current_cycle > afl->orig_queued_items) ||
                     (afl->old_seed_selection && !afl->queue_cur)))
        {
            ...
            if (unlikely(afl->old_seed_selection))
            {
                ...
                afl->afl_queue_cur = afl->queue_buf[afl->current_entry];
                ...
                if (unlikely(seek_to))
                {
                    ...
                    afl->afl_queue_cur = afl->queue_buf[seek_to];
                    seek_to = 0;
                }
            }
                         ...
                         ))
            {
                ...
                if (afl->use_splicing)
                {
                    ...
                    case 1:
                        ...
                        */
                        afl->expand_havoc = 2;
                        if (afl->cmplog_lvl && afl->cmplog_lvl < 2)
                            afl->cmplog_lvl = 2;
                        break;
                    case 2:
        ...
        do
        {
            ...
            if (afl->cx_rechoose)
            {
                afl->choose_afl_or_cx = cx_choose_afl_or_cx(afl);
            }
            if (afl->choose_afl_or_cx == 1)
            {
                afl->queue_cur = afl->afl_queue_cur;
            }
            else if (afl->choose_afl_or_cx == 2)
            {
                afl->cx_queue_cur = cx_choose_from_cx(afl);
                if (!afl->cx_queue_cur)
                {
                    FATAL();
                }
                ...
                {
                    afl->queue_cur = afl->cx_queue_cur;
                }
            }
            else
            {
            ...
            if (unlikely(!afl->stop_soon && exit_1))
            {
                afl->stop_soon = 2;
            }
...
stop_fuzzing:
    ...
    if (frida_afl_preload)
    {
        ck_free(frida_afl_preload);
    }
