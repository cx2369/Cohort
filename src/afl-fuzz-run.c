...
fuzz_run_target(afl_state_t *afl, afl_forkserver_t *fsrv, u32 timeout)
{
    ...
    fsrv_run_result_t res = cx_afl_fsrv_run_target(fsrv, timeout, &afl->stop_soon, afl->cx_trace_bits, afl->cx_map_size);
        ...
        LIST_FOREACH(&afl->custom_mutator_list, struct custom_mutator, {
            if (el->afl_custom_post_process)
            {
                ...
                if (post_process_skipped)
                {
                    ...
                    if (skip_at)
                    {
                        memcpy(mem_trimmed, (u8 *)mem, skip_at);
                    }
...
u8 cx_calibrate_case(afl_state_t *afl, struct queue_entry *q, u8 *use_mem, u32 handicap, u8 from_queue)
{
    u8 fault = 0, new_bits = 0, var_detected = 0, hnb = 0;
    u64 start_us, stop_us, diff_us;
    s32 old_sc = afl->stage_cur, old_sm = afl->stage_max;
    ...
    if (!from_queue || afl->resuming_fuzz)
    {
        use_tmout = MAX(afl->fsrv.exec_tmout + CAL_TMOUT_ADD, afl->fsrv.exec_tmout * CAL_TMOUT_PERC / 100);
    }
    ...
    if (unlikely(afl->shm.cmplog_mode))
    {
        ...
        if (!afl->non_instrumented_mode && !afl->stage_cur && !count_bytes(afl, afl->fsrv.trace_bits))
        {
            fault = FSRV_RUN_NOINST;
            goto abort_calibration;
        }
    ...
    for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max; ++afl->stage_cur)
    {
        ...
        if (q->exec_cksum != cksum)
        {
            ...
            if (q->exec_cksum)
            {
                ...
                for (i = 0; i < afl->fsrv.map_size; ++i)
                {
                    if (unlikely(!afl->var_bytes[i]) && unlikely(afl->first_trace[i] != afl->fsrv.trace_bits[i]))
                    {
                        afl->var_bytes[i] = 1;
                        afl->virgin_bits[i] = 0;
                    }
    ...
    if (!q->factor_us_len)
    {
        q->factor_us_len = q->exec_us * q->len;
    }
    ...
    // if (!q->factor_us_len_bit)
    // {
    //     double log_value = sqrt((double)(q->bitmap_size + 1));
    //     q->factor_us_len_bit = (uint64_t)(q->exec_us * q->len * (100.0 / log_value));
    // }

    afl->total_bitmap_size += q->bitmap_size;
    ++afl->total_bitmap_entries;

    cx_update_bitmap_score(afl, q);

    if (!afl->non_instrumented_mode && !fault && !new_bits)
    {
        fault = FSRV_RUN_NOBITS;
    }
...
abort_calibration:
    ...
    {
        //  cal failed
        q->exec_us = UINT32_MAX;
        if (!q->factor_us_len)
        {
            q->factor_us_len = UINT32_MAX;
        }
        // if (!q->factor_us_len_bit)
        // {
        //     q->factor_us_len_bit = UINT32_MAX;
        // }
    }
...
void sync_fuzzers(afl_state_t *afl)
{
    ...
    if (unlikely(afl->afl_env.afl_no_sync))
    {
        return;
    }
    ...
    if (!sd)
    {
        PFATAL("Unable to open '%s'", afl->sync_dir);
    }
    ...
    while ((sd_ent = readdir(sd)))
    {
        ...
        if (likely(afl->is_secondary_node))
        {
            ...
            { // an elected temporary main node

                if (likely(res == 0))
                { // there is another main node? downgrade.
...
u8 trim_case(afl_state_t *afl, struct queue_entry *q, u8 *in_buf)
{
    ...
    {
        FATAL("didn't deal with this part");
    ...
    if (q->exec_cksum == 0)
    {
        ...
        classify_counts(&afl->fsrv);
        q->exec_cksum = hash64(afl->fsrv.trace_bits, afl->fsrv.map_size, HASH_CONST);
        fault = 0;
    }
...
u8 cx_trim_case(afl_state_t *afl, struct queue_entry *q, u8 *in_buf)
{
    u8 needs_write = 0, fault = 0;
    u32 orig_len = q->len;
    u64 trim_start_us = get_cur_time_us();
    ...
    fault = 0;
    cx_set_queue_hnodeinfo_for_trim(afl->cx_trace_bits);
    orig_hnodeinfo_cksum = cx_get_hnodeinfo_cksum(afl->cx_trace_bits);
    if (q->exec_cksum == 0)
    {
        classify_counts(&afl->fsrv);
        q->exec_cksum = hash64(afl->fsrv.trace_bits, afl->fsrv.map_size, HASH_CONST);
    ...
    {
        u32 remove_pos = remove_len;
        sprintf(afl->stage_name_buf, "cxtrim %s/%s", u_stringify_int(val_bufs[0], remove_len), u_stringify_int(val_bufs[1], remove_len));
        afl->stage_cur = 0;
        afl->stage_max = q->len / remove_len;
        ...
        while (remove_pos < q->len)
        {
            ...
            ++afl->trim_execs;
            hnodeinfo_cksum = cx_get_hnodeinfo_cksum(afl->cx_trace_bits);
            classify_counts(&afl->fsrv);
            cksum = hash64(afl->fsrv.trace_bits, afl->fsrv.map_size, HASH_CONST);
            if (hnodeinfo_cksum == orig_hnodeinfo_cksum && cksum == q->exec_cksum)
            {
                u32 move_tail = q->len - remove_pos - trim_avail;
                q->len -= trim_avail;
                len_p2 = next_pow2(q->len);
