                       ...
                       struct tainted **taints)
{
    ...
    orig_hit_cnt = afl->orig_queued_items + afl->saved_crashes;
    ...
    u64 cx_start_time1 = get_cur_time();
    u64 cx_cur_time1 = 0;
    u8 cx_label = 0;
           ...
           afl->stage_cur < afl->stage_max)
    {
        ...
        if (afl->stage_cur % 500 == 0 && cx_label == 0)
        {
            cx_cur_time1 = get_cur_time();
            if ((cx_cur_time1 - cx_start_time1) > 3 * 60 * 1000)
            {
                cx_label = 1;
            }
        }
        ...
        if (unlikely(++afl->stage_cur % screen_update == 0))
        {
            show_stats(afl);
        };
    }
    ...
    new_hit_cnt = afl->orig_queued_items + afl->saved_crashes;
    ...
    */
    ...
    if (afl->cmplog_enable_transform && (lvl & LVL3))
    {
        ...
        if (use_num && ((u64)num == pattern || !num))
        {
            ...
            if (unlikely(its_fuzz(afl, new_buf, len, status)))
            {
                return 1;
            }
        }
        ...
        repl == changed_val)
    {
        ...
        else
        {
...
#else
            {
                ...
                if (ilen <= its_len && ilen > 1)
                {
                    ...
                    if (*status == 1)
                    {
                        memcpy(cbuf + idx, (char *)&new_vall, ilen);
                    }
#endif
        ...
        if (do_call)
        {
            ...
            if (ilen <= its_len && ilen > 1)
            {
                ...
                if (*status == 1)
                {
                    memcpy(cbuf + idx, (char *)&new_val, ilen);
                }
#endif
                memcpy(buf + idx, tmpbuf, ilen);
            };
                   ...
                   u32 len, u32 lvl, struct tainted *taint)
{
    ...
    for (i = 0; i < loggeds; ++i)
    {
        ...
        for (idx = 0; idx < len; ++idx)
        {
            ...
            {
                cx_cur_time1 = get_cur_time();
                if ((cx_cur_time1 - cx_start_time1) > 1 * 60 * 1000)
                {
                    idx = len;
                }
            }
...
#ifdef WORD_SIZE_64
            if (is_n)
            { // _ExtInt special case including u128
    ...
    if (afl->pass_stats[key].total < 0xff)
    {
        afl->pass_stats[key].total++;
    }
                              ...
                              u32 len, u8 lvl, u8 *status)
{
    ...
    if (lvl & LVL3)
    {
        ...
        if (memcmp(changed_val, repl, its_len) != 0)
        {
            return 0;
        }
...
#ifdef CMPLOG_COMBINE
    if (afl->orig_queued_items + afl->saved_crashes > orig_hit_cnt + 1)
    {
