    ...
    Module &M, DomTreeCallback DTCallback, PostDomTreeCallback PDTCallback)
{
    ...
    */
    ...
    if (getenv("AFL_LLVM_DICT2FILE_NO_MAIN"))
    {
        autodictionary_no_main = 1;
    }
    ...
    if (!map_addr)
    {
        AFLMapPtr = new GlobalVariable(M, PointerType::get(Int8Tyi, 0), false, GlobalValue::ExternalLinkage, 0, "__afl_area_ptr");
    }
    else
    {
    ...
    if (autodictionary)
    {
        for (auto &F : M)
        {
            if (!isInInstrumentList(&F, MNAME) || !F.size())
            {
                continue;
            }
                ...
                for (auto &IN : BB)
                {
                    ...
                    if ((cmpInst = dyn_cast<CmpInst>(&IN)))
                    {
                        ...
                        if (ilen && ilen->uge(0xffffffffffffffff) == false)
                        {
                            ...
                            u32 len = 0;
                            if (val > 0x10000 && val < 0xffffffff)
                                len = 4;
                            if (val > 0x100000001 && val < 0xffffffffffffffff)
                                len = 8;
                    ...
                    if ((callInst = dyn_cast<CallInst>(&IN)))
                    {
                        ...
                        if (optLen < 2 || (optLen == 2 && !thestring[1]))
                        {
                            continue;
                        }
                        ...
                        if (optLen != thestring.length())
                            optLen = thestring.length();
                        if (optLen > MAX_AUTO_EXTRA)
        ...
        (!getenv("AFL_LLVM_LTO_DONTWRITEID") || dictionary.size() || map_addr))
    {
        ...
        /*
                ...
                if (map_addr)
                {
                    GlobalVariable *AFLMapAddrFixed = new GlobalVariable(
                        M, Int64Tyi, true, GlobalValue::ExternalLinkage, 0,
                        "__afl_map_addr");
                    ConstantInt *MapAddr = ConstantInt::get(Int64Tyi,
                                                            map_addr);
                    StoreInst *StoreMapAddr = IRB.CreateStore(MapAddr,
                                                              AFLMapAddrFixed);
        ...
        */
        ...
        if (dictionary.size())
        {
            ...
            // extract put path
            std::string PutName = M.getModuleIdentifier();
            std::string PutNamePath = M.getSourceFileName();
            if (1)
            {
                llvm::SmallString<1024> PutNamePathVec = StringRef(PutNamePath);
                llvm::sys::fs::make_absolute(PutNamePathVec);
                llvm::StringRef PutNamePathRef = PutNamePathVec;
                PutNamePath = PutNamePathRef.str();
                PutNamePath = PutNamePath.substr(0, PutNamePath.find_last_of('/'));
                PutNamePath = PutNamePath + "/";
            }
            std::string dictfile;
            if (PutName.find_last_of('.') != std::string::npos)
            {
                dictfile = PutName.substr(0, PutName.find_last_of('.'));
            }
            else
            {
                FATAL("deal error; no dot?");
            }
            dictfile = PutNamePath + dictfile + "-dictfile";

            std::ofstream outfile(dictfile);
            if (!outfile)
            {
                FATAL();
            }
            ...
            for (const auto &token : dictionary)
            {
                if (offset + token.length() < 0xfffff0 && count < MAX_AUTO_EXTRAS)
                {
                    ...
                    outfile.write(token.c_str(), length);
                    offset += token.length();
                    ++count;
                }
            }
            ...
            if ((selectInst = dyn_cast<SelectInst>(&IN)))
            {
                if (!skip_next)
                {
                    ...
                    while (1)
                    {
                        ...
                        if (!vector_cnt || vector_cnt == vector_cur)
                        {
                            break;
                        }
                    }
                                                       ...
                                                       bool IsLeafFunc)
{
    ...
    if (Options.TracePCGuard)
    {
        ...
        {
            MDNode *meta_loc = MDNode::get(instr.getContext(), ConstantAsMetadata::get(CurLoc));
            instr.setMetadata("afl_global_id", meta_loc);
            break;
        }
