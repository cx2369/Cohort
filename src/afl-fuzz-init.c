...
static u8 bind_cpu(afl_state_t *afl, s32 cpuid)
{
    ...
    pset_create(&c);
    if (pset_assign(c, cpuid, NULL))
    {
        PFATAL("pset_assign failed");
    }
...
void bind_to_free_cpu(afl_state_t *afl)
{
    ...
    if (afl->cpu_to_bind != -1)
    {
        ...
        else
        {
            ...
            if (afl->fsrv.nyx_mode)
            {
                afl->fsrv.nyx_bind_cpu_id = afl->cpu_to_bind;
            }
#endif
        }
...
void read_foreign_testcases(afl_state_t *afl, int first)
{
    ...
    for (iter = 0; iter < afl->foreign_sync_cnt; iter++)
    {
        ...
        if (afl->foreign_syncs[iter].dir && afl->foreign_syncs[iter].dir[0])
        {
            ...
            if (first)
                ACTF("Scanning '%s'...", afl->foreign_syncs[iter].dir);
            time_t mtime_max = 0;
...
void perform_dry_run(afl_state_t *afl)
{
    ...
    for (idx = 0; idx < afl->queued_items; idx++)
    {
        ...
        if (afl->afl_env.afl_cmplog_only_new)
        {
            q->colorized = CMPLOG_LVL_MAX;
        }
...
#undef MSG_FORK_ON_APPLE
            ...
            if (afl->afl_env.afl_crashing_seeds_as_new_crash)
            {
                ...
                if (unlikely(fd < 0))
                {
                    PFATAL("Unable to create '%s'", crash_fn);
                }
    ...
    for (idx = 0; idx < afl->queued_items - 1; idx++)
    {
        ...
        q = afl->queue_buf[idx];
        if (!q || q->disabled || q->cal_failed || !q->exec_cksum)
        {
            continue;
        }
        u32 done = 0;
             ...
             likely(i < afl->queued_items && afl->queue_buf[i] && !done); ++i)
        {
            ...
            if (p->disabled || p->cal_failed || !p->exec_cksum)
            {
                continue;
            }
...
static void handle_existing_out_dir(afl_state_t *afl)
{
    ...
    {
        PFATAL("Unable to open '%s'", afl->out_dir);
    }
...
void setup_dirs_fds(afl_state_t *afl)
{
    ...
    if (mkdir(afl->out_dir, 0700))
    {
        ...
        if (errno != EEXIST)
        {
            PFATAL("Unable to create '%s'", afl->out_dir);
        }
    ...
    tmp = alloc_printf("%s/cxqueue", afl->out_dir);
    if (mkdir(tmp, 0700))
    {
        PFATAL("Unable to create '%s'", tmp);
    }
    ...
    if (!afl->in_place_resume)
    {
        ...
        fprintf(
            ...
            "saved_hangs, max_depth, execs_per_sec, total_execs, edges_found, cxqueues\n");
    }
    else
    {
...
void check_cpu_governor(afl_state_t *afl)
{
    ...
    if (afl->afl_env.afl_skip_cpufreq)
    {
        return;
    }
...
void get_core_count(afl_state_t *afl)
{
    ...
    if (sysctl(s_name, 2, &afl->cpu_core_count, &s, NULL, 0) < 0)
        return;
...
void setup_testcase_shmem(afl_state_t *afl)
{
    ...
    if (!map)
    {
        FATAL("BUG: Zero return from afl_shm_init.");
    }
...
void check_binary(afl_state_t *afl, u8 *fname)
{
...
#endif
        if (stat(afl->fsrv.target_path, &st))
        {
    ...
    */
    ...
    if (fd < 0)
    {
        PFATAL("Unable to open '%s'", afl->fsrv.target_path);
    }
