...
#ifndef __HAIKU__
    if (getrusage(RUSAGE_CHILDREN, &rus))
    {
        rus.ru_maxrss = 0;
    }
#endif
                            ...
                            double eps)
{
    ...
    /* Fields in the file:
       ...
       execs_per_sec, edges_found, cx_total_cxqueue_items */
    ...
    fprintf(afl->fsrv.plot_file,
            ...
            afl->plot_prev_ed, t_bytes, afl->cx_total_cxqueue_items); /* ignore errors */
...
static void check_term_size(afl_state_t *afl)
{
    ...
    if (ws.ws_row < 24 || ws.ws_col < 79)
    {
        afl->term_too_small = 1;
    }
}
...
#define IB(i) (val_buf[(i)])
    ...
    if (cur_ms - afl->start_time > 10 * 60 * 1000)
    {
        afl->run_over10m = 1;
    }
    ...
    if (unlikely(afl->afl_env.afl_statsd))
    {
                                                 ...
                                                 STATSD_UPDATE_SEC * 1000))
        {
            ...
            {
                WARNF("could not send statsd metric.");
            }
        }
    }
    ...
    //  cxsayf
    u64 tem_total_time1 = afl->total_cx_update2_time + afl->total_alias_time;
    u64 tem_total_time2 = afl->total_cx_get_mut_locs_time;
    tem_total_time1 = tem_total_time1 / 1000;
    tem_total_time2 = tem_total_time2 / 1000;
    // SAYF("(%u,%u);(%u,%u);\n",afl->pending_favored, afl->cx_pending_favored, afl->queued_items, afl->cx_valid_cxqueue_items);
    SAYF("cx:|%llu|%llu|%u|;1:%u;2:%u;3:%u;4:%u;\n", tem_total_time1, tem_total_time2,
         afl->recompile_count,afl->queued_items, afl->fuzz_afl_nums, afl->cx_valid_cxqueue_items, afl->fuzz_cx_nums);
...
#define SP20 SP10 SP10
    ...
    SAYF(bSTG bV bSTOP " corpus count : " cRST "%-5s " bSTG bV "\n",
         u_stringify_int(IB(0), afl->orig_queued_items));
    ...
    if (afl->cpu_core_count)
    {
        ...
        if (!afl->no_cpu_meter_red && cur_utilization >= 150)
        {
            cpu_color = cLRD;
        }

        if (afl->fsrv.snapshot)
        {
            spacing = snap;
        }
