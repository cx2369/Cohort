...
u8 *describe_op(afl_state_t *afl, u8 new_bits, size_t max_description_len)
{
    ...
    else
    {
            ...
            afl->current_custom_fuzz->afl_custom_describe)
        {
            ...
            if (is_timeout)
            {
                size_left -= strlen(",+tout");
            }
            if (unlikely(size_left <= 0))
                FATAL("filename got too long");
...
save_if_interesting(afl_state_t *afl, void *mem, u32 len, u8 fault)
{
    ...
    u8 fn[PATH_MAX];
    u8 *queue_fn = "";
    u8 new_bits = 0, cx_new_bits = 0, keeping = 0, res, classified = 0, is_timeout = 0,
       need_hash = 1;
    s32 fd;
    u64 cksum = 0;
    ...
    if (likely(fault == afl->crash_mode))
    {
        ...
        //  has new info in hnode
        if (afl->cx_start)
        {
            cx_new_bits = cx_has_new_bits(afl->cx_trace_bits);
            if (cx_new_bits || new_bits)
            {
                afl->cx_update_2 = 1;
                u8 *cx_queue_fn = "";
                cx_queue_fn = alloc_printf("%s/cxqueue/id:%06u", afl->out_dir, afl->cx_queue_id);
                s32 cxfd = permissive_create(afl, cx_queue_fn);
                if (likely(cxfd >= 0))
                {
                    ck_write(cxfd, mem, len, cx_queue_fn);
                    close(cxfd);
                }
                afl->cx_total_cxqueue_items++;
                afl->cx_queue_id++;
                cx_add_to_queue(afl, cx_queue_fn, len, 0);
                // cx_update_cxqueueid_hnoderuninfos(afl->cx_trace_bits, afl->cx_queue_id);
                cx_set_cur_cxid_and_hnodeinforuns(afl->cx_trace_bits, afl->cx_queue_id);
                if (!new_bits)
                {
                    classify_counts(&afl->fsrv);
                }
                cx_update_queueid_edgeid(afl->latest_cxqueue->afl_or_cx, afl->latest_cxqueue->cxid, afl->fsrv.trace_bits, afl->fsrv.map_size);
                cx_set_cur_edge_count(afl->fsrv.trace_bits);
                afl->latest_cxqueue->exec_cksum = hash64(afl->fsrv.trace_bits, afl->fsrv.map_size, HASH_CONST);
                u8 cxres = cx_calibrate_case(afl, afl->latest_cxqueue, mem, afl->queue_cycle - 1, 0);
                // cx_update_hnodeinforuns_bestcxid(afl->latest_cxqueue->cxid, afl->latest_cxqueue->factor_us_len_bit);
                // cx_update_edge_count_best_cxid(afl->latest_cxqueue->cxid, afl->latest_cxqueue->factor_us_len_bit);
                cx_update_hnodeinforuns_bestcxid(afl->latest_cxqueue->cxid, afl->latest_cxqueue->factor_us_len);
                cx_update_edge_count_best_cxid(afl->latest_cxqueue->cxid, afl->latest_cxqueue->factor_us_len);
                if (unlikely(cxres == FSRV_RUN_ERROR))
                {
                    FATAL("Unable to execute target application");
                }
                if (likely(afl->q_testcase_max_cache_size))
                {
                    queue_testcase_store_mem(afl, afl->latest_cxqueue, mem);
                }
            }
        }
        if (afl->cx_start)
        {
            if (new_bits)
            {
                ...
                if (likely(afl->start_time) && unlikely(afl->longest_find_time < cur_time - afl->last_find_time))
                {
    ...
    save_to_queue:
        ...
        cx_update_queueid_edgeid(afl->queue_top->afl_or_cx, afl->queue_top->id, afl->fsrv.trace_bits, afl->fsrv.map_size);
    ...
    case FSRV_RUN_TMOUT:
        ...
        if (likely(!afl->non_instrumented_mode))
        {
            ...
            u8 tem_hnb = has_new_bits(afl, afl->virgin_tmout);
            if (tem_hnb == 2)
            {
                if (afl->cx_start)
                {
    ...
    keep_as_crash:
        ...
        if (likely(!afl->non_instrumented_mode))
        {
            ...
            u8 tem_hnb = has_new_bits(afl, afl->virgin_crash);
            if (tem_hnb == 2)
            {
                if (afl->cx_start)
                {
    ...
    if (afl->fsrv.nyx_mode && fault == FSRV_RUN_CRASH)
    {
        ...
        if (unlikely(fd < 0))
        {
            PFATAL("Unable to create '%s'", fn_log);
        }
