...
struct queue_entry
{
    u8 not_free_trace_mini_label;
    u8 not_free_label;
    u64 factor_us_len;
    // u64 factor_us_len_bit;
    u32 cxid;
    u8 afl_or_cx;
    u64 favored_level; // favored for &1:edge, &2:better_progress, &4:state, &8:hash
...
{
    u8 ok_to_delete_useless;
    u32 undeal_delete_useless_count;
    u32 cur_mut_locs_size;
    struct queue_entry **cx_top_rated;
    u8 cx_score_changed;
    u64 cx_last_recompile_total_execs;
    u32 cx_queued_favored; //  for cov all edge and cov better progress
    u32 cx_pending_favored;
    u32 cx_pending_not_fuzzed;
    u32 cx_cov_all_edge_cxqueue_not_fuzzed;
    u8 *cx_map_tmp_buf;
    u64 total_cx_update2_time;
    u64 total_cx_get_mut_locs_time;
    u64 total_alias_time;
    u64 total_all_time;
    u64 next_cx_update2_time;
    u32 fuzz_afl_nums;
    u32 fuzz_cx_nums;
    u8 cx_reinit_table;
    u32 *cx_alias_table;
    double *cx_alias_probability;
    u8 cx_rechoose;
    u8 choose_afl_or_cx;
    struct queue_entry *afl_queue_cur;
    struct queue_entry *cx_queue_cur;
    s32 cx_shm_id;
    u32 cx_map_size;
    u8 *cx_trace_bits;
    u8 *put_recompile_path;
    u32 afl_final_loc;
    u32 recompile_count;
    u64 total_recompile_ms;
    uint32_t cx_start;
    u8 *virgin_bits_for_edge_time;
    u32 cx_queue_id; //  [0,x]
    u8 cx_update_1;  //  after new cx queue or new afl queue
    u8 cx_update_2;  //  after new cx queue  or new edge
    struct queue_entry *latest_cxqueue;
    u32 orig_queued_items;

    struct queue_entry **cx_total_cxqueue_buf;
    struct queue_entry **cx_valid_cxqueue_buf;
    struct queue_entry **cx_cov_all_hnode_cxqueue_buf;
    struct queue_entry **cx_cov_all_edge_cxqueue_buf;
    struct queue_entry **cx_cov_better_progress_cxqueue_buf;
    struct queue_entry **cx_cov_all_state_cxqueue_buf;
    struct queue_entry **cx_cov_all_hash_cxqueue_buf;
    struct queue_entry **cx_cov_all_edge_count_cxqueue_buf;
    struct queue_entry **cx_mix_queue_buf;
    struct queue_entry **cx_alias_table_queue_buf;

    u32 cx_total_cxqueue_items; //  [1,x]
    u32 cx_valid_cxqueue_items; //  [1,x]
    u32 cx_cov_all_hnode_cxqueue_items;
    u32 cx_cov_all_edge_cxqueue_items;        //  [1,x]
    u32 cx_cov_better_progress_cxqueue_items; //  [1,x]
    u32 cx_cov_all_state_cxqueue_items;       //  [1,x]
    u32 cx_cov_all_hash_cxqueue_items;        //  [1,x]
    u32 cx_cov_all_edge_count_cxqueue_items;
    u32 cx_mix_queue_items; //  afl and cx valid queue
    u32 cx_alias_table_queue_items;

    // u32 cx_total_cxqueue_unfuzzed;
    // u32 cx_valid_cxqueue_unfuzzed;
    // u32 cx_cov_all_edge_cxqueue_unfuzzed;
    // u32 cx_cov_better_progress_cxqueue_unfuzzed;
    // u32 cx_cov_all_state_cxqueue_unfuzzed;
    // u32 cx_cov_all_hash_cxqueue_unfuzzed;
...
void cx_add_to_queue(afl_state_t *, u8 *, u32, u8);
void destroy_queue(afl_state_t *);
void update_bitmap_score(afl_state_t *, struct queue_entry *);
void cx_update_bitmap_score(afl_state_t *, struct queue_entry *);
void cull_queue(afl_state_t *);
void cx_update_1(afl_state_t *);
void cx_update_2(afl_state_t *);
int cxcompare1(const void *, const void *);
u32 calculate_score(afl_state_t *, struct queue_entry *);
u32 cx_calculate_score(afl_state_t *, struct queue_entry *);
double cx_calculate_weight(afl_state_t *, struct queue_entry *);
...
void discover_word_2(u8 *ret, u64 *current, u64 *virgin, u64 *trace_bits_for_edge_time_64);
#else
...
u8 has_new_bits_2(afl_state_t *, u8 *, u8 *);
u8 has_new_bits_unclassified(afl_state_t *, u8 *);
u8 has_new_bits_unclassified_2(afl_state_t *, u8 *, u8 *);
#ifndef AFL_SHOWMAP
...
u8 cx_calibrate_case(afl_state_t *, struct queue_entry *, u8 *, u32, u8);
u8 trim_case(afl_state_t *, struct queue_entry *, u8 *);
u8 cx_trim_case(afl_state_t *, struct queue_entry *, u8 *);
...
static inline u32 cx_rand_below(afl_state_t *afl, u32 limit)
{
    ...
    if (afl->cur_mut_locs_size == 0)
    {
        return unbiased_rnd % limit;
    }

    u32 tem_p = 0;
    double tem_ratio = (double)afl->cur_mut_locs_size / limit;
    tem_ratio *= 3;
    tem_ratio *= 100;
    tem_p = (uint32_t)(tem_ratio);
    if (tem_p > 66)
    {
        tem_p = 66;
    }

    if ((unbiased_rnd % 100) < tem_p)
    {
        u32 my_rand_num = cx_get_random_in_cur_mut_locs();
        if (my_rand_num < limit)
        {
            return my_rand_num;
        }
        else
