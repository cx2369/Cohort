...
static u8 __cx_area_initial[CX_DEFAULT_MAP_BYTES];
static u8 *__afl_area_ptr_dummy = __afl_area_initial;
static u8 *__afl_area_ptr_backup = __afl_area_initial;

u8 *__afl_area_ptr = __afl_area_initial;
u8 *__cx_area_ptr = __cx_area_initial;
u8 *__afl_dictionary;
u8 *__afl_fuzz_ptr;
...
u16 cxstrcmp(u8 *str1, u8 *str2)
{
    ...
    for (u64 i = 0; i < 512; i++)
    {
        ...
        if ((str1[0] & 0x0F) == (str2[0] & 0x0F))
        {
            count++;
        }
        if ((str1[0] & 0xF0) == (str2[0] & 0xF0))
        {
            count++;
        }
        str1++;
...
u16 cxstrncmp(u8 *str1, u8 *str2, u64 n)
{
    u16 count = 0;
    for (u64 i = 0; i < 512; i++)
    {
        ...
        if ((str1[i] & 0x0F) == (str2[i] & 0x0F))
        {
            count++;
        }
        if ((str1[i] & 0xF0) == (str2[i] & 0xF0))
        {
            count++;
        }
    }
...
static void __afl_map_shm(void)
{
    ...
    char *cx_id_str = getenv(CX_SHM_ENV_VAR);
    ...
    if (cx_id_str)
    {
        u32 cx_shm_id = atoi(cx_id_str);
        __cx_area_ptr = (u8 *)shmat(cx_shm_id, NULL, 0);
        if (__cx_area_ptr == (void *)-1)
        {
            _exit(1);
        }
    }
...
__attribute__((constructor())) void __afl_auto_init(void)
{
    ...
    if (__afl_already_initialized_init)
    {
        return;
    }
...
__attribute__((constructor(EARLY_FS_PRIO))) void __early_forkserver(void)
{
    ...
    if (getenv("AFL_EARLY_FORKSERVER"))
    {
        __afl_auto_init();
    }
}
...
__attribute__((constructor(CTOR_PRIO))) void __afl_auto_early(void)
{
    ...
    if (__afl_already_initialized_early)
        return;
...
__attribute__((constructor(1))) void __afl_auto_second(void)
{
    ...
    if (__afl_already_initialized_second)
        return;
...
__attribute__((constructor(0))) void __afl_auto_first(void)
{
    ...
    if (__afl_already_initialized_first)
        return;
...
void __sanitizer_cov_trace_pc_guard_init(uint32_t *start, uint32_t *stop)
{
    ...
    if (__afl_already_initialized_forkserver)
    {
        ...
        else
        {
            ...
            while (start < stop)
            {
                ...
                if (unlikely(++offset >= __afl_final_loc))
                {
                    offset = 5;
                }
            }
    ...
    if (__afl_final_loc < 4)
        __afl_final_loc = 4; // we skip the first 5 entries
                        ...
                        uint8_t size)
{
    ...
    if (likely(!__afl_cmp_map))
        return;
    if (unlikely(arg1 == arg2 || size > __afl_cmplog_max_len))
        return;
...
static int area_is_valid(void *ptr, size_t len)
{
    ...
    if (unlikely(!ptr || __asan_region_is_poisoned(ptr, len)))
    {
        return 0;
    }
...
void __cmplog_rtn_hook_strn(u8 *ptr1, u8 *ptr2, u64 len)
{
    ...
    if (likely(!__afl_cmp_map))
        return;
    if (unlikely(!len || len > __afl_cmplog_max_len))
        return;
    ...
    if (len1 <= 32)
        len1 = area_is_valid(ptr1, len1 + 1);
    if (len1 > __afl_cmplog_max_len)
        len1 = 0;

    int len2 = strnlen(ptr2, len0);
    if (len2 <= 32)
        len2 = area_is_valid(ptr2, len2 + 1);
    if (len2 > __afl_cmplog_max_len)
        len2 = 0;
