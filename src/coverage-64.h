...
inline u32 skim(const u64 *virgin, const u64 *current, const u64 *current_end)
{
    ...
    for (; current < current_end; virgin += 4, current += 4)
    {
        ...
        if (unlikely(current[0] && classify_word(current[0]) & virgin[0]))
            return 1;
        if (unlikely(current[1] && classify_word(current[1]) & virgin[1]))
            return 1;
        if (unlikely(current[2] && classify_word(current[2]) & virgin[2]))
            return 1;
        if (unlikely(current[3] && classify_word(current[3]) & virgin[3]))
            return 1;
    }
