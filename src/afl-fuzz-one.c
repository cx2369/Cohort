...
u8 fuzz_one_original(afl_state_t *afl)
{
    ...
    afl->cx_rechoose = 0;
    if (afl->queue_cur->afl_or_cx == 1)
    {
        if (likely(afl->pending_favored))
        {
            if ((afl->queue_cur->fuzz_level || !afl->queue_cur->favored) && likely(rand_below(afl, 100) < SKIP_TO_NEW_PROB))
            {
                return 1;
            }
        }
        else if (!afl->non_instrumented_mode && !afl->queue_cur->favored && afl->queued_items > 10)
        {
    ...
    else if (afl->queue_cur->afl_or_cx == 2)
    {
        //  nothing
    }
    else
    ...
    cx_clear_cur_mut_locs();
    afl->cur_mut_locs_size = 0;

    if (afl->cx_start && afl->queue_cur->fuzz_level > 0 && afl->queue_cur->fuzz_level < 6)
    {
        uint32_t tem_queue_id = 0;
        if (afl->queue_cur->afl_or_cx == 1)
        {
            tem_queue_id = afl->queue_cur->id;
        }
        else if (afl->queue_cur->afl_or_cx == 2)
        {
            tem_queue_id = afl->queue_cur->cxid;
        }
        else
        {
            FATAL();
        }
        cx_reset_cur_mut_locs(afl->queue_cur->afl_or_cx, tem_queue_id, afl->queue_cur->len);
        afl->cur_mut_locs_size = cx_get_cur_mut_locs_size();
        afl->cur_mut_locs_size = afl->cur_mut_locs_size * 4; //  one loc means 4 bytes / 32 bits.
    }
    ...
    if (unlikely(afl->queue_cur->cal_failed))
    {
        ...
        if (afl->queue_cur->cal_failed < CAL_CHANCES)
        {
            ...
            {
                res = cx_calibrate_case(afl, afl->queue_cur, in_buf, afl->queue_cycle - 1, 0);
            }
            else
            {
                FATAL();
                 ...
                 !afl->disable_trim))
    {
        ...
        {
            u8 res = cx_trim_case(afl, afl->queue_cur, in_buf);
            orig_in = in_buf = queue_testcase_get(afl, afl->queue_cur);
            if (unlikely(res == FSRV_RUN_ERROR))
            {
                FATAL("Unable to execute target application");
    ...
    if (afl->queue_cur->afl_or_cx == 1)
    {
        if (unlikely(afl->shm.cmplog_mode && afl->queue_cur->colorized < afl->cmplog_lvl && (u32)len <= afl->cmplog_max_filesize))
        {
            if (unlikely(len < 4))
            {
                ...
                if (afl->queue_cur->favored || afl->cmplog_lvl == 3 ||
                    (afl->cmplog_lvl == 2 &&
                     (afl->queue_cur->tc_ref ||
                      afl->fsrv.total_execs % afl->orig_queued_items <= 10)) ||
    ...
    else if (afl->queue_cur->afl_or_cx == 2)
    {
        if (unlikely(afl->shm.cmplog_mode && afl->queue_cur->colorized < afl->cmplog_lvl && (u32)len <= afl->cmplog_max_filesize))
        {
            ...
            {
                if (((afl->queue_cur->favored_level & 16) || (afl->queue_cur->favored_level & 1)) ||
                    afl->cmplog_lvl == 3 ||
                    (afl->cmplog_lvl == 2 &&
                     (afl->queue_cur->tc_ref ||
                      afl->fsrv.total_execs % afl->orig_queued_items <= 10)) ||
...
#define FLIP_BIT(_ar, _b)                         \
    ...
    for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max; ++afl->stage_cur)
    {
        ...
        if (is_det_timeout(before_det_time, 0))
        {
            goto custom_mutator_stage;
        }
...
skip_extras:
    ...
    {
        mark_as_det_done(afl, afl->queue_cur);
    }
...
custom_mutator_stage:
    ...
    if (likely(!afl->custom_mutators_count))
    {
        goto havoc_stage;
    }
...
havoc_stage:
    ...
    if (unlikely(afl->stage_max < HAVOC_MIN))
    {
        afl->stage_max = HAVOC_MIN;
    }
    ...
    orig_hit_cnt = afl->orig_queued_items + afl->queued_items + afl->saved_crashes;

    havoc_queued = afl->queued_items;

    u64 cx_havoc_queued = afl->orig_queued_items;
    u64 cx_havoc_queued2 = afl->cx_total_cxqueue_items;
    ...
    for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max; ++afl->stage_cur)
    {
        ...
        retry_havoc_step:
        {
            ...
            case MUT_FLIPBIT:
            {
                ...
                u32 off = cx_rand_below(afl, temp_len);
                out_buf[off] ^= 1 << bit;
            ...
            case MUT_INTERESTING8:
            {
                ...
                out_buf[cx_rand_below(afl, temp_len)] = interesting_8[item];
                break;
            }
            ...
            case MUT_INTERESTING16:
            {
                ...
                // *(u16 *)(out_buf + rand_below(afl, temp_len - 1)) = interesting_16[item];
                *(u16 *)(out_buf + cx_rand_below(afl, temp_len - 1)) = interesting_16[item];
            ...
            case MUT_INTERESTING16BE:
            {
                ...
                strcat(afl->mutation, afl->m_tmp);
#endif
                // *(u16 *)(out_buf + rand_below(afl, temp_len - 1)) = SWAP16(interesting_16[item]);
                *(u16 *)(out_buf + cx_rand_below(afl, temp_len - 1)) = SWAP16(interesting_16[item]);
            ...
            case MUT_INTERESTING32:
            {
                ...
                // *(u32 *)(out_buf + rand_below(afl, temp_len - 3)) = interesting_32[item];
                *(u32 *)(out_buf + cx_rand_below(afl, temp_len - 3)) = interesting_32[item];
            ...
            case MUT_INTERESTING32BE:
            {
                ...
                strcat(afl->mutation, afl->m_tmp);
#endif
                // *(u32 *)(out_buf + rand_below(afl, temp_len - 3)) = SWAP32(interesting_32[item]);
                *(u32 *)(out_buf + cx_rand_below(afl, temp_len - 3)) = SWAP32(interesting_32[item]);
            ...
            case MUT_ARITH8_:
            {
                ...
                out_buf[cx_rand_below(afl, temp_len)] -= item;
                break;
            }
            ...
            case MUT_ARITH16_:
            {
                ...
                u32 pos = cx_rand_below(afl, temp_len - 1);
                item = 1 + rand_below(afl, ARITH_MAX);
            ...
            case MUT_ARITH32_:
            {
                ...
                u32 pos = cx_rand_below(afl, temp_len - 3);
                item = 1 + rand_below(afl, ARITH_MAX);
            ...
            case MUT_RAND8:
            {
                ...
                u32 pos = cx_rand_below(afl, temp_len);
            ...
            case MUT_CLONE_COPY:
            {
                ...
                if (likely(temp_len + HAVOC_BLK_XL < MAX_FILE))
                {
                    ...
                    u32 clone_to = cx_rand_below(afl, temp_len);
            ...
            case MUT_OVERWRITE_COPY:
            {
                ...
                do
                {
                    ...
                    copy_to = cx_rand_below(afl, temp_len - copy_len + 1);
            ...
            case MUT_OVERWRITE_FIXED:
            {
                ...
                u32 copy_to = cx_rand_below(afl, temp_len - copy_len + 1);
                u32 strat = rand_below(afl, 2);
            ...
            case MUT_BYTEADD:
            {
                ...
                out_buf[cx_rand_below(afl, temp_len)]++;
                break;
            }
            ...
            case MUT_FLIP8:
            {
                ...
                out_buf[cx_rand_below(afl, temp_len)] ^= 0xff;
                break;
            }
            ...
            case MUT_SWITCH:
            {
                ...
                do
                {
                    ...
                    switch_to = cx_rand_below(afl, temp_len);
            ...
            case MUT_DEL:
            {
                ...
                u32 del_from = cx_rand_below(afl, temp_len - del_len + 1);
            ...
            case MUT_SHUFFLE:
            {
                ...
                u32 off = cx_rand_below(afl, temp_len - len + 1);
            ...
            case MUT_ASCIINUM:
            {
                ...
                u32 off = cx_rand_below(afl, temp_len), off2 = off, cnt = 0;
            ...
            case MUT_EXTRA_OVERWRITE:
            {
                ...
                if (unlikely(extra_len > temp_len))
                {
                    goto retry_havoc_step;
                }

                // u32 insert_at = rand_below(afl, temp_len - extra_len + 1);
                u32 insert_at = cx_rand_below(afl, temp_len - extra_len + 1);
#ifdef INTROSPECTION
            ...
            case MUT_EXTRA_INSERT:
            {
                ...
                u32 insert_at = cx_rand_below(afl, temp_len + 1);
#ifdef INTROSPECTION
            ...
            case MUT_SPLICE_OVERWRITE:
            {
                ...
                u32 new_len;
                u8 *new_buf;
                if (afl->cx_mix_queue_items)
                {
                    u32 tem_try = 0;
                    do
                    {
                        tid = rand_below(afl, afl->cx_mix_queue_items);
                        tem_try++;
                        if (tem_try > 100)
                        {
                            break;
                        }
                    } while (afl->cx_mix_queue_buf[tid]->len < 4 || ((afl->queue_cur->id == afl->cx_mix_queue_buf[tid]->id) && (afl->queue_cur->cxid == afl->cx_mix_queue_buf[tid]->cxid)));

                    target = afl->cx_mix_queue_buf[tid];
                    new_len = target->len;
                    new_buf = queue_testcase_get(afl, target);
                }
                else
        ...
        if (cx_havoc_queued2 != afl->cx_total_cxqueue_items)
        {
            cx_havoc_queued2 = afl->cx_total_cxqueue_items;
            u64 tem_start_time = get_cur_time();
            u32 not_match_percentage = 0;
            u32 cx_tem_small_len = temp_len < len ? temp_len : len;
            u32 max_iterations = cx_tem_small_len / 4 * 4; //  once deal 32bits = 4bytes
            for (i = 0; i < max_iterations; i += 4)
            {
                if (*(uint32_t *)(out_buf + i) != *(uint32_t *)(in_buf + i))
                {
                    not_match_percentage++;
                }
            }
            if (not_match_percentage < 64 && not_match_percentage * 4 * 4 < cx_tem_small_len) //  diff 4bytes nums  less than  32 and diff bytes less than total bytes / 4
            {
                cx_set_label1(1); //  disable new hnode_newinfo_for_mut_locs
                for (i = 0; i < max_iterations; i += 4)
                {
                    if (*(uint32_t *)(out_buf + i) != *(uint32_t *)(in_buf + i))
                    {
                        uint32_t temp_outbuf = 0;
                        temp_outbuf = *(uint32_t *)(out_buf + i);
                        *(uint32_t *)(out_buf + i) = *(uint32_t *)(in_buf + i);
                        cx_set_cur_mut_loc(i);
                        common_fuzz_stuff(afl, out_buf, temp_len);
                        *(uint32_t *)(out_buf + i) = temp_outbuf;
                    }
                }
                cx_set_label1(0);
            }
            u64 tem_end_time = get_cur_time();
            afl->total_cx_get_mut_locs_time += (tem_end_time - tem_start_time);
        }
        ...
        if (havoc_queued != afl->queued_items || cx_havoc_queued != afl->orig_queued_items)
        {
            if (perf_score <= afl->havoc_max_mult * 100)
            {
                afl->stage_max *= 2;
            ...
            havoc_queued = afl->queued_items;
            cx_havoc_queued = afl->orig_queued_items;
        }
    }

    new_hit_cnt = afl->orig_queued_items + afl->queued_items + afl->saved_crashes;
...
abandon_entry:
    ...
    if (!afl->stop_soon && !afl->queue_cur->cal_failed && !afl->queue_cur->was_fuzzed && !afl->queue_cur->disabled)
    {
        ...
        {
            if (afl->cx_pending_not_fuzzed == 0)
            {
                FATAL();
            }
            ...
            if (afl->queue_cur->favored_level & 1)
            {
                if (afl->cx_cov_all_edge_cxqueue_not_fuzzed == 0)
                {
                    FATAL();
                }
    ...
    if (1)
    {
        ...
        cx_update_hnode_fuzzed_count(afl->queue_cur->afl_or_cx, tem_queue_id);
        cx_update_edgeid_fuzzcount(afl->queue_cur->afl_or_cx, tem_queue_id);
    }
                ...
                for (i = 0; i < use_stacking; ++i)
                {
                    ...
                    default:
                    {
                        ...
                        else
                        {
                            ...
                            if (unlikely(afl->ready_for_splicing_count < 2))
                                break;
...
u8 fuzz_one(afl_state_t *afl)
{
    ...
    */
    ...
    if (unlikely(key_val_lv_1 == -1))
    {
        key_val_lv_1 = 0;
    }
    if (likely(key_val_lv_2 == -1))
    {
        key_val_lv_2 = 0;
    }
